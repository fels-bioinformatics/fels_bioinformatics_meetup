...
ggplot(mtcars, aes(x = wt, y = mpg)) + 
  geom_point(size = 3) +
  geom_abline(slope = -5.34, intercept = 37.3, 
...
```{r}
aov(Sepal.Length ~ Species, data = iris) %>% TukeyHSD() 
...
### Miscellaneous from End of Class

R and R-squared are not the same thing


```{r}
lm(Sepal.Width ~ Sepal.Length, data = iris) %>% glance()
```

```{r}
iris %$% cor(Sepal.Width, Sepal.Length)
```
