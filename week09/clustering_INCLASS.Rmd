...
```{r}
# alternatice PCA function
iris %>% select(-Species) %>% princomp() %>% summary()
```
...
```{r}
# with scaling
iris %>% select(-Species) %>% prcomp(scale = TRUE) %>% augment(iris) -> iris_pca_scale
```
...
```{r}
#ris %>% select(-Species) %>% Rtsne(check_duplicates = FALSE) %>% tidy()
```
...
cbind(iris, iris_tsne$Y) %>% 
  as_tibble() %>% 
  rename(tSNE1 = `1`, tSNE2 = `2`) -> iris_tbl_w_tsne
```
...
```{r}
iris_tbl_w_tsne %>% select(tSNE1, tSNE2) %>% kmeans(3) %>% augment(iris_tbl_w_tsne) -> iris_tsne_kmeans
```

```{r}
ggplot(iris_tsne_kmeans, aes(x = tSNE1, y = tSNE2, color = .cluster, shape = Species)) +
  geom_point(size = 3) +
  theme_classic() +
  theme(axis.title = element_text(size = 14), legend.title = element_text(size = 14))
```
