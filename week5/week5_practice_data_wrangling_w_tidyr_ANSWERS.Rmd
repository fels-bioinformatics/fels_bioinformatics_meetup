...
Because it's complicated and hasn't been covered so far use this mutate command `mutate(code = stringr::str_replace(code, 'newrel', 'new_rel'))` to fix the problem. Pipe into the mutate command and then to the separate command you ran before and that should fix the problem.
...
All the columns look good now! Their values all belong to the variable indicated in the column name. However, the case_type column only has 'new' in it. That makes this columnn unnecessary, so drop the case_type column in the chunk below.
...
One last problem - as I'm sure you've noticed, there are a lot of NAs in the count column! They need to be dealt with in some way. Try both methods, just dropping NAs, and replacing NAs with some reasonable value
