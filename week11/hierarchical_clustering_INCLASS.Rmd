...
Default distance in `dist()` is "euclidean", but you can select different distances by supplying a different argument to `method` inside `dist()`

```{r}
clust_demo %>% as.data.frame() %>% column_to_rownames('label') -> clust_demo_dataframe

dist(clust_demo_dataframe, method = 'manhattan')
```

You can also supply `hclust()` with a correlation matrix. You can you can correlate all rows first, then calculate the distances of the correlations and give that to `hclust()`

```{r}
iris %>% select(-Species) %>% t() %>% cor(.) %>% dist(.) -> iris_dist

hclust(iris_dist)

plot(hclust(iris_dist))

library(pheatmap)
pheatmap(iris_dist)

iris %>% select(-Species) %>% t() %>% as.matrix() %>% pheatmap()
```
...
str(clust_demo_hclust)
```

Default clustering method in `hclust()` is "complete", but like with `dist()` you can select the method.

```{r}
hclust(clust_demo_dist, method = 'Ward.D2')
```
...
# that to geom_segment() to plot the branches of the tree
  geom_segment(data = clust_demo_ggdendro$segments, 
...
# to geom_text() to label the ends of the branches
  geom_text(data = clust_demo_ggdendro$labels, 
            aes(x = x, y = y, label = label), vjust = 1) +
  theme_dendro()
```

```{r}
clust_demo_ggdendro$segments
```

<br><br>

From Jozef: Plot other shapes of cluster dendrograms

```{r}
library(ape)
plot(as.phylo(clust_demo_hclust), type = 'fan')
```
